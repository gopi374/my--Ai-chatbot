const API_KEY = "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";
...
async function fetchGroqData(messages) {
  try {
    const response = await fetch(API_URL, {
      method: "POST",
      headers: {
        Authorization: `Bearer ${API_KEY}`, // Fixed string interpolation
    ...
    if (!response.ok) {
      ...
      throw new Error(`HTTP error! status: ${response.status}, body: ${errorBody}`); // Fixed string interpolation
    }
...
appendMessage(""); // This line can be removed if not needed
